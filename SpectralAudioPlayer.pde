...
void draw() {
  ...
  if (errorOccured) {
    ...
    text("Make sure your audio file is either .wav .mp3 or .aiff and not more than 16 bit", width / 2, height / 2 + 100);
  }
  if (makingWaveForm) text("Generating waveform...", width / 2, height /2);
  if (!fileLoaded && !errorOccured) text("Loading file...", width / 2, height /2);
  if (fileLoaded) {
      if (keyPressed && keyCode == 39) player.cue(player.position() + 5000); //skip forward one second
